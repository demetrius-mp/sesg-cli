...
from sesg_cli.database.models import SLR, SearchStringPerformance
from sesg_cli.database.util.results_queries import ResultQuery, SideQueries
...
def _statistics_tab(results: dict[str, dict], excel_writer: pd.ExcelWriter, strategies: list[str]) -> None:
...
def save_xlsx(excel_writer: pd.ExcelWriter, results: dict[str, dict], slr: str, strategies: list[str]):
...
def get_results(slr: str, metrics: list[str]) -> tuple[dict, list[str]]:
    ...
    with Session() as session:
        strategies_used = SearchStringPerformance.get_strategies_used(
            strategies_used_queries, check_review_query, session)

        results: dict = {}
        strategies: list[str] = []

        for strategies_set in product(*strategies_used.values()):
            tes = strategies_set[0][0]
            sws = strategies_set[1][0]
            ...
            results.update(SearchStringPerformance.get_results(
                queries, session))

            strategies.append(f"{tes}-{sws}")

        results.update(SearchStringPerformance.get_results(
            result_query.get_qgs_query(),
            session))
